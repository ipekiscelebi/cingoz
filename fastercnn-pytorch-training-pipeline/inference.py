...
def parse_opt():
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help='image size for file labeling'
    )
    parser.add_argument(
        '--batch',
        default=None,
        required=True,
        type=int,
        help='batch size for file labeling'
    )
    parser.add_argument(
        '--epoch',
        default=None,
...
def main(args):
    ...
    DEVICE = args['device']
    OUT_DIR = set_infer_dir(args['weights'].split('/')[1],args['image'],args['batch'],args['epoch'])
