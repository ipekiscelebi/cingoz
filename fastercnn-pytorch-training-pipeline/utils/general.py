...
def index_dir(dir,file_name):
    dir_num = 1
    for i in os.listdir(dir):
        if file_name in i:
            dir_num += 1
    new_dir =f'{dir}/{file_name}_{dir_num}'
    os.makedirs(new_dir,exist_ok=True)
    return new_dir

def set_export_dir(model_name,file_name):
    """
    This function creates a named directory in `weights/` and returns the path.
    
    """
    dir = f"weights/{model_name}"
    os.makedirs(dir, exist_ok=True)
    file_dir = os.path.join(dir,file_name)
    try:
        os.makedirs(file_dir)
    except OSError:
        print(f"Directory '{file_dir}' can not be created, either it already exits or you don't have permission to create it.")
        print("Indexing the directory...")
        file_dir = index_dir(dir,file_name)
    return file_dir
    

def set_infer_dir(weight,threshold):
    """
    ...
    next_dir_num = 0
    for file in os.listdir('outputs/inference/'):
        if 'res_' in file:
            old_dir_num = int(file.split('_')[1])
            if old_dir_num >= next_dir_num:
                next_dir_num = old_dir_num + 1
    new_dir_name = f"outputs/inference/res_{next_dir_num}_{weight}_threshold_{threshold}"
...
def set_training_dir(dir_name=None):
    ...
    if dir_name:
        for file in os.listdir('outputs/training/'):
            if dir_name in file:
                ...
                if old_dir_num >= next_dir_num:
                    next_dir_num = old_dir_num + 1
        if duplicate_flag:
            new_dir_name = f"outputs/training/{dir_name}_{next_dir_num}"
        else:
